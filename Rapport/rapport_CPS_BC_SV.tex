...
Pour chaque service \emph{xx}, nous avons créé les fichiers suivants :  \\

\begin{itemize}
\item \emph{xxService.java}  une interface déclarant chaque méthode associée aux constructeurs, aux observateurs et aux opérateurs du service en question.  \\
\item \emph{xxDecorator.java}  une classe abstraite d'enrobage réalisant l'interface \emph{xxService} en utilisant le design dattern décorateur.\\
\item \emph{xxContract.java}  une classe héritant de \emph{xxDecorator} et réalisant l'action voulue pour le service en question sur une implémentation de \emph{xxService}, tout en permettant la vérification de la justesse des pré-conditions, post-conditions, et invariants.  \\
\item \emph{xxImpl.java}  une classe "concrete" qui implémente l'interface \emph{xxService} et réalise les actions liées à la logique du jeu. \\
\item \emph{xxFailImpl.java}  une seconde classe qui implémente le jeu, mais qui ne respecte pas les diverses conditions établies par la spécification ( = une implémentation buguée ) \\
\end{itemize}
...
\begin{itemize}
\item Une implémentation respectant la spécification, sur laquelle nous pouvons lancer les tests, correspondant aux classes \emph{xxImpl.java}
\item Une implémentation buguée, correspondant aux classes \emph{xxFailImpl.java}
\item Une application exécutable, permettant de lancer réellement le jeu, qui correspond aux classes de \emph{ihm}, et dont la classe principale est \emph{RiverCityGraphic.java}. \\
\end{itemize}
